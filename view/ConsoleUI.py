...
from view.menu_items.NewNote import AddNote
from view.menu_items.DelNote import DeleteNote
from view.menu_items.ENote import EditNote
from view.menu_items.Exit import Exit
from view.menu_items.FDate import FindDate
...
class ConsoleUI(View):
    ...
    def edit_note(self):
        selection = input("номер заметки для редактирования ")
        size = self.__presenter.get_notes_size()
    ...
    def delete_note(self):
        selection = input("номер заметки для удаления ")
        size = self.__presenter.get_notes_size()
        if Validator.is_valid(selection, size):
    ...
    def add_note(self):
        note = Note()
        note.header = input("заголовок заметки ")
        note.text = input("текст заметки ")
    ...
    def exit(self):
        if self.__presenter.get_edited_flag:
            print("Изменилось")
            confirmation = input("Данные изменены. Сохранить изменения y/n: ")
